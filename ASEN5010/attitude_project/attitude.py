...
class Attitude():
    ...
    def attitude_propagation(self,X0:list[float],tspan:list[float],u_func=None,dt:float=1)->dict[float]:
        """ 
        ...
        for index,t in enumerate(t_vec[0:-1],0):
            ...
            if u_func is not None:
                u = u_func(t,X[index])  ### REVISIT WHEN CONTROL IS FURTHER ELABORATED UPON
            else:
                u = np.array([0,0,0])
            u_list[index + 1] = u
