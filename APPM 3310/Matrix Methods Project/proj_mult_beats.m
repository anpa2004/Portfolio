...
title(sprintf("Transmembrane Voltage for %g beats",n));
grid minor;
...
%% Calculating Steady State values of APD[] 
% This assumes that the steady state is the mean. This probably isn't true
...
for i = first:last
    APDs(i-(first-1)) = APDsc{i};
end

% Plotting APD* to ensure it seems reasonable. 
figure()
plot(10*values,APDs,"Linewidth",1);
grid minor;
xlabel('APD^{[]}_{*}');
ylabel('APD_{*} [ms]')
title("APD_{*} values for Different Percentages of Restabilization")
...
for i = 1:n
    for j = first:last
        Z(i,j-(first-1)) = APDv{j}(i)-APDsc{j}; % i is beat number, j is APD value
    end
end
